...
use crate::ankigen::db_model::deck::{Deck, DeckConfig};
use crate::ankigen::db_model::model::Model;
...
impl Collection {
    fn serialize_models<S>(models: &[(usize, Model)], serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let mut result = serializer.serialize_map(Some(models.len()))?;
        for (model_time, model) in models {
            let model_json = serde_json::to_value(model).map_err(serde::ser::Error::custom)?;
            result.serialize_entry(&model_time.to_string(), &model_json.to_string())?;
        }
        result.end()
    }

    fn deserialize_models<'de, D>(deserializer: D) -> Result<Vec<(usize, Model)>, D::Error>
    where
        D: Deserializer<'de>,
    {
        ...
        impl<'de> Visitor<'de> for CollectionVisitor {
                ...
                V: MapAccess<'de>,
            {
                ...
                while let Some((key, value)) = map.next_entry::<String, String>()? {
                    let model_time = key.parse::<usize>().map_err(de::Error::custom)?;
                    let model: Model = serde_json::from_str(&value).map_err(de::Error::custom)?;
    ...
    fn serialize_decks<S>(decks: &[(usize, Deck)], serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let mut result = serializer.serialize_map(Some(decks.len()))?;
        for (deck_time, deck) in decks {
            let deck_json = serde_json::to_value(deck).map_err(serde::ser::Error::custom)?;
            result.serialize_entry(&deck_time.to_string(), &deck_json.to_string())?;
        }
        result.end()
    }

    fn deserialize_decks<'de, D>(deserializer: D) -> Result<Vec<(usize, Deck)>, D::Error>
    where
        D: Deserializer<'de>,
    {
        struct CollectionVisitor;
        impl<'de> Visitor<'de> for CollectionVisitor {
                ...
                V: MapAccess<'de>,
            {
                ...
                while let Some((key, value)) = map.next_entry::<String, String>()? {
                    let deck_time = key.parse::<usize>().map_err(de::Error::custom)?;
                    let deck: Deck = serde_json::from_str(&value).map_err(de::Error::custom)?;
    ...
    where
        S: Serializer,
    {
        let mut result = serializer.serialize_map(Some(deck_configs.len()))?;
        for (deck_time, deck_config) in deck_configs {
            let deck_config_json =
                serde_json::to_value(deck_config).map_err(serde::ser::Error::custom)?;
            result.serialize_entry(&deck_time.to_string(), &deck_config_json.to_string())?;
        }
        result.end()
    }
